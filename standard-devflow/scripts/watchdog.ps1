...
function Get-HbFact {
    ...
    return @{ age_min = [Math]::Round($age, 1); long = $isLong; note = $note }
}
...
function Check-States($f) {
    $age = $f.heartbeat.age_min
    $limit = if ($f.heartbeat.long) { $LongMin } else { $KillMin }
    $warn = if ($f.heartbeat.long) { $LongMin } else { $WarnMin }
    ...
    if ($age -gt $warn -and $script:State.warn -ne 'on') {
        ...
        Record-Event 'agent_stale_warning' ("心跳 {0} 分钟未更新（LONG={1}），阈值预警" -f $age, $f.heartbeat.long)
    ...
    if ($age -gt $limit -and -not $hasFacts -and $script:State.critical -ne 'on') {
        ...
        Record-Event 'agent_stale_critical' ("心跳 {0} 分钟未更新且无产出（LONG={1}），判定卡死候选" -f $age, $f.heartbeat.long)
        Write-Evidence $f
